...
export default function ImportAllPage() {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="md:flex md:items-center md:justify-between mb-6">
        <div className="min-w-0 flex-1">
          <h1 className="text-3xl font-semibold text-gray-900">Toplu Envanter İçe Aktarma</h1>
          <p className="mt-2 text-gray-700">
            Excel dosyanızı yükleyerek tüm zimmet bilgilerini toplu olarak içe aktarabilirsiniz.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        {importResults && (
          <div className={`mb-4 p-4 rounded-md ${importResults.results.errors.length > 0 ? 'bg-yellow-50 text-yellow-800' : 'bg-green-50 text-green-800'}`}>
            <p className="font-medium">{importResults.message}</p>
            {importResults.results.errors.length > 0 && (
              <div className="mt-2">
                <p className="font-medium">Hatalar:</p>
                <ul className="mt-1 list-disc pl-5 text-sm max-h-40 overflow-y-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mb-2">
            Excel dosyanız aşağıdaki sütun başlıklarını içermelidir:
          </p>
          <div className="overflow-x-auto my-4">
            <table className="min-w-full divide-y divide-gray-200 border">
              <thead>
                <tr className="bg-gray-50">
                  <th className="px-3 py-2 text-xs text-gray-700 border">Departman</th>
                  <th className="px-3 py-2 text-xs text-gray-700 border">İsim</th>
                  <th className="px-3 py-2 text-xs text-gray-700 border">Telefon Marka/Model</th>
                  <th className="px-3 py-2 text-xs text-gray-700 border">Telefon Seri no</th>
                  <th className="px-3 py-2 text-xs text-gray-700 border">Telefon IMEI</th>
                  <th className="px-3 py-2 text-xs text-gray-700 border">Bilgisayar Marka/Model</th>
                  <th className="px-3 py-2 text-xs text-gray-700 border">Hostname</th>
                  <th className="px-3 py-2 text-xs text-gray-700 border">Bilgisayar Seri No</th>
                  <th className="px-3 py-2 text-xs text-gray-700 border">Bilgisayar UUID</th>
                </tr>
              </thead>
            </table>
          </div>
          <p className="text-sm text-gray-600">
            * İsim alanı zorunludur. Diğer alanlar boş bırakılabilir.
          </p>
        </div>
